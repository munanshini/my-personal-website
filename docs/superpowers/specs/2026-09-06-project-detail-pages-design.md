...
将现有 hash 单页作品集改造成可直接访问、可分享、可预渲染的多路径静态网站。新增三个项目详情页，内容先使用带 `TODO: 待替换为最终文案` 注释的结构化占位数据。保留现有配色、字体、动效、首页布局，以及 `SpotlightCard`、`SectionHeading`、`BorderGlow` 的设计语言。
...
OSS + CDN 是当前主站的目标托管；GitHub Pages 保留为备用入口。两者从同一源码构建，但使用不同 `base`：阿里云为 `/`，GitHub Pages 为 `/my-personal-website/`。canonical 和 Open Graph URL 始终使用正式域名 `https://zhangnanai.com`。
...
| `/` | 首页 |
| `/work` | 作品列表 |
| `/work/ai-ide` | AI IDE 研发助手 |
| `/work/warehouse-scheduling` | 智能仓储调度系统 |
| `/work/smart-sales-center` | 房企智慧案场销讲与客户接待系统 |
| `/words` | 内容 |
| `/now` | 现在 |
| `/contact` | 联系 |
...
不配置 SPA fallback。未知路径可使用独立 `404.html`，但不返回首页 HTML。这样每条项目 URL 在阿里云与 GitHub Pages 都能直接打开、刷新，并在“查看网页源代码”中看到对应正文。
...
- `WorkItem` 新增 `slug` 与 `detail`，移除 `detailLabel`。
...
作品列表卡片中的“项目档案”改成可访问的详情链接；保持现有 hover、焦点状态与项目卡片主体排版。详情页加载和返回使用现有 `PageTransition`。
...
增加一张 1200×630 的统一站点主视觉作为详情页兜底分享图，所有图片补充准确 `alt`。构建脚本从路由表生成 8 条 URL 的 `sitemap.xml` 与指向它的 `robots.txt`。

## 双目标构建与发布

构建脚本：

- `build:aliyun`：根路径 `base=/`，输出用于 OSS 的 `dist/`。
- `build:pages`：子路径 `base=/my-personal-website/`，输出用于 GitHub Pages 的 `dist/`。
...
1. 路由和旧 hash 兼容单元测试。
2. 数据结构、项目链接、详情页语义层级与核心内容测试。
3. 元数据与 JSON-LD 的页面级测试。
4. `npm test`、`npm run build:aliyun`、`npm run build:pages`。
5. 对两个构建产物检查 8 个实体 HTML、不同页面标题/描述、详情正文、sitemap 和 robots。
6. 以本地静态服务器检查 `/work/ai-ide` 直达与刷新，以及桌面和移动端布局。
7. 发布后分别检查 `zhangnanai.com/work/ai-ide` 与 GitHub Pages 备用地址；阿里云生产切换须在资源、域名、证书和 DNS 均核对后进行。
