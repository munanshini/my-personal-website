...
export function ContentFeed({ items }: { items: WordItem[] }) {
  return (
    <section id="words" className="border-t border-black/10 bg-paper px-5 py-16 text-ink transition-colors duration-700 sm:px-8 md:py-32">
      <div className="mx-auto max-w-canvas">
        ...
        <div className="mt-10 grid gap-4 sm:mt-16 lg:grid-cols-3">
          {items.map((item) => (
            <article key={item.index} className={`${item.accent} group flex min-h-[340px] flex-col justify-between rounded-[2rem] p-5 sm:min-h-[430px] sm:p-8`}>
              ...
              <div>
                ...
                <h3 className="text-3xl font-semibold leading-[1.02] tracking-[-0.055em] sm:text-5xl">{item.title}</h3>
