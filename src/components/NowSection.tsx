...
export function NowSection({ items }: { items: NowItem[] }) {
  ...
  return (
    <section id="now" className="border-t border-white/15 bg-[#111417] px-5 py-24 text-white transition-colors duration-700 sm:px-8 md:py-32">
      <div className="mx-auto max-w-canvas">
        <SectionHeading index="04" eyebrow="NOW" title="此刻，我在关注什么。" description="一个不断变化的小栏目，用来记录最近的产品问题、创作与生活。" inverse />
        <div className="mt-20 min-h-[690px] divide-y divide-white/15 border-y border-white/15">
          {visibleItems.map((item, index) => (
            <div key={item.type} className="grid gap-4 py-7 md:grid-cols-[120px_150px_1fr_auto] md:items-center">
