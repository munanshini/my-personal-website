          ...
          {projects.map((project, index) => (
            ...
            >
              <div className="relative z-10 flex h-full flex-col">
                <header className={`flex items-start justify-between gap-4 border-b pb-5 text-[10px] font-semibold uppercase tracking-[0.18em] opacity-70 sm:text-xs ${'company' in project && project.company === '华为' ? 'border-current/[.07]' : 'border-current/20'}`}>
                  <span>{project.number} / {project.category}</span>
                  <span>{'companyPeriod' in project ? project.companyPeriod : project.year}</span>
                </header>

                {'company' in project && (
                  <div className={`mt-5 flex flex-wrap items-baseline justify-between gap-3 border-b pb-5 ${project.company === '华为' ? 'border-current/[.07]' : 'border-current/15'}`}>
                    <div>
                ...
                <div className={`grid flex-1 gap-10 py-10 ${index === 0 ? 'lg:grid-cols-[1.1fr_.9fr] lg:items-end' : ''}`}>
                  ...
                  <div className={`self-end rounded-3xl border bg-white/10 p-5 backdrop-blur-sm sm:p-6 ${'company' in project && project.company === '华为' ? 'border-current/[.105]' : 'border-current/15'}`}>
                    ...
                    <ul className={`mt-7 space-y-3 border-t pt-5 ${'company' in project && project.company === '华为' ? 'border-current/[.105]' : 'border-current/15'}`}>
                ...
                <footer className={`flex flex-wrap items-end justify-between gap-5 border-t pt-5 ${'company' in project && project.company === '华为' ? 'border-current/[.07]' : 'border-current/20'}`}>
                  <div className="flex flex-wrap gap-2">
                    {project.tags.map((tag) => (
                      <span key={tag} className={`rounded-full border px-3 py-1.5 text-[10px] font-medium uppercase tracking-[0.1em] ${'company' in project && project.company === '华为' ? 'border-current/[.14]' : 'border-current/20'}`}>{tag}</span>
                    ))}
                  </div>
